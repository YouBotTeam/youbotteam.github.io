    ...
    if (!conversationContainer.querySelector(".container-pillole")) {
        if (!document.querySelector(".container-custom")) {
            ...
            pillole.forEach((pillola) => {
                ...
                contentLi.innerHTML = `
                                <button type="button" class="collapsible"><img class="pillole-icon" src="${pillola.multimedia.find(
                    (multi) => multi.type === "icon"
                ).link
                    }"/><h1 class="pillole-title">${pillola.title
                    }</h1></button>
                                <div class="content">
                                    <p>${pillola.content}</p>
                                    <iframe src="${pillola.multimedia.find(
...
function getHintComplete() {
    ...
    myHeaders.append("Authorization", "Basic dXRlbnRlOjQ0YzJkYmUyYzI0NzE5YWFlNjRlZDQyOTg5YzllM2YxZTUwNDQ3NGQwZjQ4NzFiYzI2YmFiNjY5NWY5NWQ5MTI=");
    myHeaders.append("Content-Type", "application/json");

    var raw = JSON.stringify({
        "text": "la colonna iniz. agenzia"
    });
    ...
    fetch("https://weai.it/service/recommend/cattolica", requestOptions)
...
function autocomplete(arr) {
    ...
    autocompleteElement.addEventListener("click", function () {
        createListToComplete(this, currentFocus, arr);
    });
...
function createListToComplete(context, currentFocus, arr) {
    var a,
        b,
        i,
        val = context.value;
